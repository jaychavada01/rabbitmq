...
const { EXCHANGES, QUEUES, CONSUMER_OPTIONS } = require("./constants");
...
async function consumeMessages(
  ...
  try {
    ...
    console.log(
      `[Consumer] ✓ DLX asserted: ${EXCHANGES.DEAD_LETTER_EXCHANGE.name}`,
    );
    ...
    if (dlqConfig) {
      ...
      // Bind DLQ to DLX
      const dlqRoutingKey =
        queueConfig.options.arguments["x-dead-letter-routing-key"];
    ...
    await channel.bindQueue(queueConfig.name, exchangeConfig.name, routingKey);
    console.log(
    ...
    await channel.consume(
      ...
      async (msg) => {
        if (!msg) {
          console.warn(
            "[Consumer] Received null message (consumer cancelled)",
          );
          return;
        }
        ...
        try {
          ...
          channel.nack(msg, false, false);
          console.log(
            "[Consumer] Message rejected due to error (sent to DLQ)",
          );
        }
      },
      {
    ...
    console.log(
      `[Consumer] Waiting for messages on queue: ${queueConfig.name}...`,
    );
  } catch (error) {
